...
ALLOWED_GOOGLE_EMAILS="${ALLOWED_GOOGLE_EMAILS:-${ALLOWED_GOOGLE_EMAIL:-}}"
OWNER_LABEL="${OWNER_LABEL:-team-finance}"

if [[ -z "$ALLOWED_GOOGLE_EMAILS" ]]; then
  read -r -p "Enter allowed Google email address(es), comma-separated: " ALLOWED_GOOGLE_EMAILS
fi

if [[ -z "$ALLOWED_GOOGLE_EMAILS" ]]; then
  echo "ALLOWED_GOOGLE_EMAILS must be provided before deploying." >&2
  exit 1
fi
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars="ALLOWED_GOOGLE_EMAILS=${ALLOWED_GOOGLE_EMAILS},ALLOW_DEV_LOGIN=0,DB_NAME=${DB_NAME},DB_USER=${DB_USER},CLOUDSQL_CONNECTION_NAME=${CONNECTION_NAME},HOST=0.0.0.0,OPENAI_MODEL=gpt-4.1-mini" \
